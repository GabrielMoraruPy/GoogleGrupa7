...
# header = ["NR. CRT", "Judet"] + [f"0{day}.03" if day < 10 else f"{day}.03" for day in range(1, 6)]
...
# print(numar_cazuri_int)
print(lista_int)
print(sum(lista_int))
# total = 0
# for judet, caz in data.items():
#     if isinstance(caz, int) or (
#             hasattr(caz, 'isdigit') and caz.isdigit()
#     ):
#         total += int(caz)

# print(total)
# suma_totala = ["Total", ""] + [sum(int(cazuri)]
...
sheet.append(suma_totala)
# suma_totala = ["Total", ""] + [sum([float(data[judet][i]) for judet in data]) for i in range(len(urls))]
